...
remote_state {
  ...
  config = {
    bucket         = "terraform-state-${local.config.inputs.aws_region}-${local.config.inputs.environment}"
    key            = "${local.config.inputs.chamber_name}/${local.config.inputs.aws_region}/${path_relative_to_include()}/terraform.tfstate"
